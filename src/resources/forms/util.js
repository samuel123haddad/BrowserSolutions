...
function xmlTextR(node, buf, cdata) {
  if (node.nodeType == DOM_TEXT_NODE) {
	...
	//#SER#Fix - avoid empty element to textarea, iframe and canvas
    if (node.childNodes.length == 0 && node.nodeName !== 'textarea' && node.nodeName !== 'iframe' && node.nodeName !== 'canvas' && node.nodeName !== 'div' && node.nodeName !== 'span') {
      buf.push('/>');
    } else {
      buf.push('>');
