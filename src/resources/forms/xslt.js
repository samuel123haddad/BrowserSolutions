...
function xsltProcessContext(input, template, output) {
    var node, nodes, select, value, mode, vname, nameexpr, topDoc, nj, i, j, c;
    ...
    if (nodename.length == 1 || nodename[0] != 'xsl') {
        ...
        switch(nodename[1]) {
            ...
            case 'apply-templates':
                ...
                //#Ser# top is reserved in chrome, and modified order
                topDoc = template.ownerDocument.documentElement;
                while (topDoc.nodeType !== DOM_ELEMENT_NODE && topDoc.nodeName != 'xsl:stylesheet' && topDoc.nodeName != 'xsl:transform' && topDoc.nextSibling) {
                    topDoc = topDoc.nextSibling;
                }
                ...
                var templates = [];
                for (i = 0; i < topDoc.childNodes.length; ++i) {
                    c = topDoc.childNodes[i];
                ...
                for (j = 0; j < sortContext.contextSize(); ++j) {
                    nj = sortContext.nodelist[j];
            ...
            case 'call-template':
                vname = xmlGetAttribute(template, 'name');
                topDoc = template.ownerDocument.documentElement;
                while (topDoc.nodeType !== DOM_ELEMENT_NODE && topDoc.nodeName != 'xsl:stylesheet' && topDoc.nodeName != 'xsl:transform' && topDoc.nextSibling) {
                    topDoc = topDoc.nextSibling;
                }
            ...
            case 'copy-of':
                ...
                if (value.type == 'node-set') {
                    ...
                    node = domCreateTextNode(outputDocument, value.stringValue());
            ...
            case 'element':
                nameexpr = xmlGetAttribute(template, 'name');
                vname = xsltAttributeValue(nameexpr, input);
                node = domCreateElement(outputDocument, vname);
                domAppendChild(output, node);
                xsltChildNodes(input, template, node);
                break;
            ...
            case 'value-of':
                select = xmlGetAttribute(template, 'select');
                var xvalue = xpathEval(select, input);
                if (!xvalue || typeof xvalue.stringValue != "function")
                    debugger;
                value = xvalue.stringValue();
...
function xsltVariable(input, template, override) {
    var vname = xmlGetAttribute(template, 'name');
    ...
    if (override || !input.getVariable(vname)) {
        input.setVariable(vname, value);
    }
}
...
function xsltPassThrough(input, template, output, outputDocument) {
    if (template.nodeType == DOM_TEXT_NODE) {
        ...
        for (var i = 0; i < template.attributes.length; ++i) {
            ...
            if (a) {
                var vname = a.nodeName;
                var value = xsltAttributeValue(a.nodeValue, input);
                domSetAttribute(node, vname, value);
            }
        }
        domAppendChild(output, node);
        xsltChildNodes(input, template, node);
...
function xsltMatch(match, context) {
    if (! context.node.matchCache) {
        context.node.matchCache = new Object();
    }
    if (typeof context.node.matchCache[match] === 'boolean')
        return context.node.matchCache[match];
    var ret;
    ...
    if (expr.steps && !expr.absolute && expr.steps.length == 1 &&
        ...
        while (!ret && node) {
            var econtext = context.clone(node,0,[node]);
            var result = expr.evaluate(econtext).nodeSetValue();
            econtext.release();
    ...
    context.node.matchCache[match] = ret;
    return ret;
}
