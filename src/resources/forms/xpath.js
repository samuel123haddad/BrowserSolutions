...
function PStack() {
    this.l = 0;
    this.m = new Array(100);
}

PStack.prototype.isEmpty = function() {
    return this.l == 0;
}

PStack.prototype.push = function(obj) {
    if (this.m.length-1 == this.l)
        this.m = this.m.concat(new Array(100));
    this.m[this.l++] = obj;
}
...
{
    this.assign(node, opt_position, opt_nodelist, opt_parent,
        opt_caseInsensitive, opt_ignoreAttributesWithoutValue,
        opt_returnOnFirstMatch, opt_ignoreNonElementNodesForNTA);
}

ExprContext.pstack = new PStack();

ExprContext.prototype.assign = function(node, opt_position, opt_nodelist, opt_parent,
  opt_caseInsensitive, opt_ignoreAttributesWithoutValue,
  opt_returnOnFirstMatch, opt_ignoreNonElementNodesForNTA) {
  this.node = node;
...
ExprContext.prototype.clone = function(opt_node, opt_position, opt_nodelist) {
  ...
  if (ExprContext.pstack.isEmpty()) {
      ...
      ret = ExprContext.pstack.pop();
      ret.assign(opt_node || this.node,
          typeof opt_position != 'undefined' ? opt_position : this.position,
          opt_nodelist || this.nodelist, this, this.caseInsensitive,
          this.ignoreAttributesWithoutValue, this.returnOnFirstMatch,
          this.ignoreNonElementNodesForNTA);
...
ExprContext.prototype.release = function() {
    ExprContext.pstack.push(this);
}

ExprContext.releaseAll=function() {
    while (! ExprContext.pstack.isEmpty()) {
        ExprContext.pstack.pop();
    }
}
...
LocationExpr.prototype.evaluate = function(ctx) {
  ...
  if (! start.xpathEval) {
      start.xpathEval = new Object();
  }
  if ((typeof this.cacheKey != 'undefined') && (! /toolbar/.test(this.cacheKey)) && (typeof start.xpathEval[this.cacheKey] != 'undefined')) {
      return start.xpathEval[this.cacheKey];
  }
  var nodes = [];
  xPathStep(nodes, this.steps, 0, start, ctx);
  var r = new NodeSetValue(nodes);
  if (typeof this.cacheKey != 'undefined') 
      start.xpathEval[this.cacheKey] = r;
  return r;
};
  ...
  if (ctx.returnOnFirstMatch && !s.hasPositionalPredicate) {
    ...
    for (var i = 0; i < nLength; ++i) {
      ...
      for (var j = 0; j < pLength; ++j) {
        var clist = ctx.clone(n, i, nodelist);
        if (!s.predicate[j].evaluate(clist).booleanValue()) {
          clist.release();
...
UnionExpr.prototype.evaluate = function(ctx) {
  ...
  // Redefine to use hashmap
  if (nodes2.length > 0) {
      for(var j=0; j < nodes2.length; ++j) {
          var n = nodes2[j];
          if (nodes1.indexOf(n) == -1)
              nodes1.push(n);
      }
  }
...
// token and 2 on the stack. Simplify this!
