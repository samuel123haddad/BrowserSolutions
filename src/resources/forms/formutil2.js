/**
 ** Este arquivo sobreescreve algumas funcoes do formutil.js deve ser chamado logo a seguir do mesmo.
 **/

var FORMSFORMUTILJS="forms/formutils2.js";
var xXMLHttpRequest = window.XMLHttpRequest;

//-----------------------------------------------------------------------------
XNode.prototype.selectSingleNode=function(path) {
	var rst = xpathParse(path).evaluate(new ExprContext(this));
	if ((rst instanceof NodeSetValue) && (rst.value.length > 0))
		return rst.value[0];
	return rst.stringValue();
}

XNode.prototype.transformNode = function(xslDoc) {
	return xsltProcess(this, xslDoc);
}
...
XNode.loadXML = function(url) {
	var xhr = new xXMLHttpRequest();
	xhr.open("GET", url, false);
	xhr.send(null);
	var rxml = xhr.responseText;
	if (! rxml || /^\s*$/.test(rxml))
		return null;
	return xml = xmlParse(rxml);
}

Document.prototype.selectSingleNode=function(xpath) {
	var xpe = new XPathEvaluator();
	var nsResolver = xpe.createNSResolver(this.ownerDocument == null ? this.documentElement : this.ownerDocument.documentElement);
	var result = xpe.evaluate(xpath, this, nsResolver, XPathResult.FIRST_ORDERED_NODE_TYPE, null);
	if (result && result.resultType == XPathResult.FIRST_ORDERED_NODE_TYPE) {
                var r = result.singleNodeValue;
                if (r)
...
if (Document != top.Document) {
    top.Document.prototype.selectSingleNode = Document.prototype.selectSingleNode;
}
...
function user_initForm(ctx)
{
	try {
		var height=ctx.getAttribute("height");
		var gHeight=ctx.getAttribute("gHeight");
		var width=ctx.getAttribute("width");
		var gWidth=ctx.getAttribute("gWidth");
...
function user_initDetailArea(ctx)
{
	var dtlnode = this.args.length > 0 ? this.args[0].evaluate(ctx) : undefined;
	var fldnodes = this.args.length > 1 ? this.args[1].evaluate(ctx) : undefined;
	var rownodes = this.args.length > 2 ? this.args[2].evaluate(ctx) : undefined;
	try {
		if (dtlnode) dtlnode = dtlnode.value[0];
		if (fldnodes) fldnodes = fldnodes.value[0];
		if (rownodes) rownodes = rownodes.value[0];
		var curNode=dtlnode
		rowCnt=-1
		dtlCols=1;
		colOffset=1;
		...
		dtlHdrRows = parseInt(rownodes.getAttribute("row"));
		if (fldnodes.length < 1) return new StringValue("");
		var firstNode = fldnodes;

		firstRow=parseInt(firstNode.getAttribute("row"));
		colOffset=parseInt(rownodes.getAttribute("col"));
...
//-----------------------------------------------------------------------------
function user_getFormCols()
{
	return new NumberValue(nCols);
}
//-----------------------------------------------------------------------------
function user_getFormColWidth()
{
	return new NumberValue(nColWidth);
}
//-----------------------------------------------------------------------------
function user_getFormWidth()
{
	var width = parseInt(ctx.node.getAttribute("width"),10);
	if (! isFinite(width))
		width=0;
	return new NumberValue(width * nColWidth);
}
//-----------------------------------------------------------------------------
function user_getFormRows()
{
	return new NumberValue(nRows);
}
//-----------------------------------------------------------------------------
function user_getFormHeight()
{
	var height = parseInt(ctx.node.getAttribute("height"),10);
	if (! isFinite(height))
		height=0;
	return new NumberValue(height * nRowHeight);
}
//-----------------------------------------------------------------------------
function user_getFormRowHeight()
{
	return new NumberValue(nRowHeight);
}
//-----------------------------------------------------------------------------
function user_getColPos(ctx)
{
	var strDefault="0px";
	try {
		var curNode=ctx.node;
		var colVal=curNode.getAttribute("col");
		if (colVal==null) return new StringValue(strDefault);
		...
		return new StringValue("" + calcColVal +"px");

	} catch (e) {
		return new StringValue(strDefault);
	}
}
//-----------------------------------------------------------------------------
function user_getRowCol(ctx)
{
	var strDefault="5px";
	try {
		var col=parseInt(ctx.node.getAttribute("col"))
		return new StringValue("" + ((col-colOffset)*nColWidth) + "px")
...
//-----------------------------------------------------------------------------
function user_getRowWidth()
{
	return new NumberValue(rowWidth);
}
//-----------------------------------------------------------------------------
function user_getRowPos(ctx)
{
	try {
		var curNode=ctx.node;
		...
		return new StringValue(rowVal != 0 ? "" + (rowVal*nRowHeight) + "px" : "3px");

	} catch(e) {
		return new StringValue("3px");
	}
}
//-----------------------------------------------------------------------------
function user_getTextWidth(ctx)
{
	var strDefault="13px";
	try {
		var curNode=ctx.node;
		if (curNode==null) return new StringValue(strDefault);

		var strSize = curNode.getAttribute("sz");
		if (strSize==null) return new StringValue(strDefault);
		var size = parseInt(strSize);
		if(size==1) return new StringValue(strDefault);
...
//-----------------------------------------------------------------------------
function user_getLabelWidth(ctx)
{
	var strDefault="13px";
	try {
		var curNode=ctx.node;
		...
		return new StringValue("" + calcWidth + "px")
...
//-----------------------------------------------------------------------------
function user_getButtonWidth(ctx)
{
	var curNode=ctx.node;
	if(curNode.getAttribute("sz") && curNode.getAttribute("sz")!="")
	{
		...
		if (sz < 3)
			sz += 0.3;
		return new StringValue("" + (sz*nColWidth) + "px");
	}

	var caption=curNode.getAttribute("btnnm")
	return new StringValue("" + ((caption+"").length*nColWidth) + "px");
}
//-----------------------------------------------------------------------------
function user_getSelectButtonCol(ctx)
{
	var bScroll = this.args.length > 1 ? this.args[1].evaluate(ctx).booleanValue() : false;
	var strDefault="5px";
	try {
		var calcColVal;
		var curNode=ctx.node;
		...
		return new StringValue(y == 1 ? "" + (calcColVal+3) + "px" : "" + calcColVal +"px");
...
function user_getRectTop(ctx)
{
	try {
		...
		return new StringValue("" + (rowVal*nRowHeight) + "px");
...
function user_getRectLeft(ctx)
{
	try {
		...
		if (col == 0.35 && curNode.parentNode.nodeName == "form")
			col=1;
		return new StringValue("" + (col*nColWidth) + "px")

	} catch (e) { }
	return new StringValue("")
}
//-----------------------------------------------------------------------------
function user_getRectHeight(ctx)
{
	try {
		var curNode=ctx.node;
		var ht=parseInt(curNode.getAttribute("height"))
		if (ht==1) return new StringValue("1px");
		...
		return new StringValue("" + (ht*nRowHeight) + "px");

	} catch (e) { }
	return new StringValue("1px");
}
//-----------------------------------------------------------------------------
function user_getRectWidth(ctx)
{
	try {
		var curNode=ctx.node;
		...
		return new StringValue("" + ((size-0.8)*nColWidth)  + "px");

	} catch (e) { }
	return new StringValue("1px");
}
//-----------------------------------------------------------------------------
function user_getRectLabelClass(ctx)
{
	try {
		var curNode=ctx.node;
		if ( curNode.parentNode.nodeName=="detail" )
			return new StringValue("rectLabelDtl");
		if ( curNode.parentNode.nodeName=="tab" )
		{
			if ( curNode.parentNode.parentNode.parentNode.nodeName == "form" )
				return new StringValue("rectLabelTab");
		}
		if ( curNode.parentNode.parentNode )
		{
			if (curNode.parentNode.parentNode.getAttribute("det")=="1")
				return new StringValue("rectLabelTabSub");
		...
		if (parseInt(curNode.getAttribute("height"))==1)
			...
			return new StringValue("rectLabel");

	} catch (e) { }
	return new StringValue("rectLabel");
}
//-----------------------------------------------------------------------------
function user_getHeight(ctx)
{
	try {
		var curNode=ctx.node;
		var height=0
		...
		if(curNode.nodeName=="detail")
		{
			...
			return new StringValue("" + ((height*nRowHeight)+2) + "px");
		}
		...
		if (curNode.nodeName == "tabregion" && curNode.getElementsByTagName("detail") != null)
			return new StringValue("" + ((height*nRowHeight) + 8) + "px");
		else
			return new StringValue("" + (height*nRowHeight) + "px");

	} catch (e) { }
	return new StringValue("");
}
//-----------------------------------------------------------------------------
function user_getDtlRowPos(ctx)
{
	try {
		var rowVal=parseInt(ctx.node.getAttribute("row"))
		return new StringValue("" + (rowVal-dtlHdrRows)*nRowHeight + "px");

	} catch(e) { }
	return new StringValue("");
}
//-----------------------------------------------------------------------------
function user_getDtlColPos(nodelist)
{
	try {
		var curNode=ctx.node;
		var curCol=curNode.getAttribute("col");
		if (curCol && curCol != "0")
			return new StringValue(("" + (parseInt(curCol)*nColWidth) + "px"));
		else if	(curNode.parentNode.nodeName != "form")
			return new StringValue("4px");

	} catch (e) { }
	return new StringValue("" + (nColWidth) + "px");
}
//-----------------------------------------------------------------------------
function user_getDtlWidth()
{
	return new StringValue(dtlWidth);
}
//-----------------------------------------------------------------------------
function user_getDtlHdrHeight()
{
	return new StringValue("" + dtlHdrRows*nRowHeight + "px");
}
//-----------------------------------------------------------------------------
function user_getDtlBodyHeight()
{
	return new StringValue("" + Math.ceil(dtlBodyRows/dtlCols)*nRowHeight + "px");
}
//-----------------------------------------------------------------------------
function user_getDtlRowCnt()
{
	return new NumberValue(rowCnt);
}
//-----------------------------------------------------------------------------
function user_incDtlRowCnt()
{
	rowCnt++;
	return new StringValue("");
}
//-----------------------------------------------------------------------------
function user_getTabRegionCol(ctx)
{
	try {
		var curNode=ctx.node;
		var curCol=curNode.getAttribute("col");
...
//-----------------------------------------------------------------------------
function user_getTabRegionWidth(ctx)
{
	try {
		var curNode=ctx.node;
		...
		return new StringValue("" + rgnWidth + "px");

	} catch (e) { }
	return new StringValue("" + (nCols*nColWidth) + "px");
}
//-----------------------------------------------------------------------------
function user_setTableTR(nodelist, pos, child)
{
	try {
		var pos = this.args.length > 1 ? this.args[1].evaluate(ctx).numberValue() : undefined;
		var child = this.args.length > 2 ? this.args[2].evaluate(ctx).numberValue() : undefined;
		...
		return new NumberValue(bTR);

	} catch (e) { }
	return new NumberValue(0);
}
//-----------------------------------------------------------------------------
function user_dataBind(ctx)
{
	try {
		var curNode=ctx.node;
		...
		return new StringValue(db);

	} catch (e) { }
	return new StringValue("");
}
//-----------------------------------------------------------------------------
function user_getParentName(ctx)
{
	try {
		var curNode = ctx.node;
		if (curNode.parentNode.nodeName=="detail")
			return new StringValue(curNode.parentNode.getAttribute("nbr"));
		if (curNode.parentNode.nodeName=="tabregion")
			return new StringValue(curNode.parentNode.getAttribute("nbr"));
		if (curNode.parentNode.nodeName=="tab")
			return new StringValue(curNode.parentNode.getAttribute("nbr"));
		if (curNode.parentNode.nodeName=="scroll")
			return new StringValue(curNode.parentNode.parentNode.getAttribute("nbr") ?
				curNode.parentNode.parentNode.getAttribute("nbr") : "form");
		if (curNode.parentNode.nodeName=="form")
			return new StringValue("form");
...
function user_tokenIsFlowChart(ctx)
{
	try {
		var formId = this.args.length > 0 ? this.args[0].evaluate(ctx).stringValue() : "";
		return new BooleanValue(formId.match(flowRE) ? true : false);

	} catch (e) { }
	return new BooleanValue(false);
}
//-----------------------------------------------------------------------------
function user_isSubTab(ctx)
{
	try {
		var curNode=ctx.node;
...
//-----------------------------------------------------------------------------
function user_isCurrentTab(ctx)
{
	var curNode=ctx.node;
...
function user_isActiveTab(ctx)
{
	...
	return new BooleanValue(tabNbr == tabActive ? true : false);
}
//-----------------------------------------------------------------------------
function user_isActiveSubTab(ctx)
{
	var curNode=ctx.node;
	var tabNbr=curNode.getAttribute("nbr");
	var isSecured = curNode.getAttribute("tabprot");
	...
	return new BooleanValue(tabNbr == tabActiveSub ? true : false);
}
//-----------------------------------------------------------------------------
function user_isLongTabString(ctx)
{
	var tabString = this.args.length > 0 ? this.args[0].evaluate(ctx) : undefined;
	return new BooleanValue((tabString.length*nColWidth) > tabMaxWide-4 ? true : false);
}
//-----------------------------------------------------------------------------
function user_getShortTabString(tabString)
{
	var tabString = this.args.length > 0 ? this.args[0].evaluate(ctx) : undefined;
	if (tabString.length <= Math.ceil(tabMaxWide/nColWidth)+3)
		return tabString;
	return new StringValue(tabString.substr(0,(tabMaxWide/nColWidth))+"...");
}

//-----------------------------------------------------------------------------
// Extendendo as funcoes soft
FunctionCallExpr.prototype.xpathfunctions['user_initForm']=user_initForm;
FunctionCallExpr.prototype.xpathfunctions['user_initTabArea']=user_initTabArea;
FunctionCallExpr.prototype.xpathfunctions['user_initDetailArea']=user_initDetailArea;
FunctionCallExpr.prototype.xpathfunctions['user_getFormCols']=user_getFormCols;
FunctionCallExpr.prototype.xpathfunctions['user_getFormColWidth']=user_getFormColWidth;
FunctionCallExpr.prototype.xpathfunctions['user_getFormRows']=user_getFormRows;
FunctionCallExpr.prototype.xpathfunctions['user_getFormRowHeight']=user_getFormRowHeight;
FunctionCallExpr.prototype.xpathfunctions['user_getColPos']=user_getColPos;
FunctionCallExpr.prototype.xpathfunctions['user_getRowCol']=user_getRowCol;
FunctionCallExpr.prototype.xpathfunctions['user_getRowWidth']=user_getRowWidth;
FunctionCallExpr.prototype.xpathfunctions['user_getRowPos']=user_getRowPos;
FunctionCallExpr.prototype.xpathfunctions['user_getTextWidth']=user_getTextWidth;
FunctionCallExpr.prototype.xpathfunctions['user_getLabelWidth']=user_getLabelWidth;
FunctionCallExpr.prototype.xpathfunctions['user_getButtonWidth']=user_getButtonWidth;
FunctionCallExpr.prototype.xpathfunctions['user_getSelectButtonCol']=user_getSelectButtonCol;
FunctionCallExpr.prototype.xpathfunctions['user_getRectTop']=user_getRectTop;
FunctionCallExpr.prototype.xpathfunctions['user_getRectLeft']=user_getRectLeft;
FunctionCallExpr.prototype.xpathfunctions['user_getRectHeight']=user_getRectHeight;
FunctionCallExpr.prototype.xpathfunctions['user_getRectWidth']=user_getRectWidth;
FunctionCallExpr.prototype.xpathfunctions['user_getRectLabelClass']=user_getRectLabelClass;
FunctionCallExpr.prototype.xpathfunctions['user_getHeight']=user_getHeight;
FunctionCallExpr.prototype.xpathfunctions['user_getDtlRowPos']=user_getDtlRowPos;
FunctionCallExpr.prototype.xpathfunctions['user_getDtlColPos']=user_getDtlColPos;
FunctionCallExpr.prototype.xpathfunctions['user_getDtlWidth']=user_getDtlWidth;
FunctionCallExpr.prototype.xpathfunctions['user_getDtlHdrHeight']=user_getDtlHdrHeight;
FunctionCallExpr.prototype.xpathfunctions['user_getDtlBodyHeight']=user_getDtlBodyHeight;
FunctionCallExpr.prototype.xpathfunctions['user_getDtlRowCnt']=user_getDtlRowCnt;
FunctionCallExpr.prototype.xpathfunctions['user_incDtlRowCnt']=user_incDtlRowCnt;
FunctionCallExpr.prototype.xpathfunctions['user_getTabRegionCol']=user_getTabRegionCol;
FunctionCallExpr.prototype.xpathfunctions['user_getTabRegionWidth']=user_getTabRegionWidth;
FunctionCallExpr.prototype.xpathfunctions['user_setTableTR']=user_setTableTR;
FunctionCallExpr.prototype.xpathfunctions['user_dataBind']=user_dataBind;
FunctionCallExpr.prototype.xpathfunctions['user_getParentName']=user_getParentName;
FunctionCallExpr.prototype.xpathfunctions['user_tokenIsFlowChart']=user_tokenIsFlowChart;
FunctionCallExpr.prototype.xpathfunctions['user_isSubTab']=user_isSubTab;
FunctionCallExpr.prototype.xpathfunctions['user_isCurrentTab']=user_isCurrentTab;
FunctionCallExpr.prototype.xpathfunctions['user_isActiveTab']=user_isActiveTab;
FunctionCallExpr.prototype.xpathfunctions['user_isActiveSubTab']=user_isActiveSubTab;
FunctionCallExpr.prototype.xpathfunctions['user_isLongTabString']=user_isLongTabString;
FunctionCallExpr.prototype.xpathfunctions['user_getShortTabString']=user_getShortTabString;
...
//	if (typeof(output) == "undefined" || output==null)
	...
	if(IEXMLSERVICE==null)
		IEXMLSERVICE=new ChromeFormService(portalWnd)
	output="xml"
	...
	// note: do not re-order _PARMS; lawformc.xsl dependent on order.
 	...
 	return lawsonPortal.formsDir+"/formhost2.htm?"+strXML
}
...
//---------------------------------------------------------------------------------------
function frmShowCalendar2(wnd, fld)
{
	// form disabled?
	if (!wnd.formState.formReady) return
	...
	if (!wnd.oDropDown)
		wnd.oDropDown=new wnd.Dropdown()
	wnd.oDropDown.show(oDate, mElement, "lawformCalendarSel2")
}
...
function ActionFrameworkService(portalWnd)
{
	...
	this.oXSL=XNode.loadXML(oXML.xml);
}
//----------------------------------------------------------------------------
...
ActionFrameworkService.prototype.buildForm=function(myXML)
{
	...
	if(xmlDoc)
		formHTM = xsltProcess(xmlDoc, xslDoc);
	return formHTM
}
...
function ChromeFormService(portalWnd)
{
    this.portalWnd=portalWnd;
    this.oXSL=XNode.loadXML(this.portalWnd.lawsonPortal.formsDir + "/lawform2.xsl");
}

//-----------------------------------------------------------------------------
ChromeFormService.prototype.loadForm=function(xpressCall)
{
    // object is used by all forms and XML here is simply the last one built
    var parms=this.portalWnd.getVarFromString("_PARMS",xpressCall);
    this.tkn=this.portalWnd.getVarFromString("_TKN",xpressCall);
    ...
    if (!myXML)
    {
        ...
        if (this.portalWnd.oFormCache
        && this.portalWnd.oUserProfile.useFormCache())
            this.portalWnd.oFormCache.saveForm(this.pdl,this.tkn,this.id,myXML);
    }

    try {
        // set root node attributes for XSL processing
        var frmNode=myXML.selectSingleNode("//form");
		if (! frmNode.setAttribute)
			debugger;
        frmNode.setAttribute("rootdir",this.rootdir);
        frmNode.setAttribute("mode",this.mode);
        frmNode.setAttribute("pdl",this.pdl);
...
//-----------------------------------------------------------------------------
ChromeFormService.prototype.loadPreview=function()
{
    // instantiate our own DOM object to protect
    // against version mismatch with Design Studio
    try {
        var myXML=Document.loadXML(this.portalWnd.lawsonPortal.preview.XMLDocument.xml);
...
//-----------------------------------------------------------------------------
ChromeFormService.prototype.buildTab=function(tabID,oXML)
{
	//::SER::Nao achei ponto onde eh colocado a tabAtiva, forcei aqui.
	tabActive = tabID;
...
//-----------------------------------------------------------------------------
ChromeFormService.prototype.buildForm=function(myXML)
{
	var formHTM=""
	xmlDoc=myXML
	xslDoc=this.oXSL
